(()=>{
  ...
  function style(){
    if(document.getElementById('thorConsultaV082Style'))return;
    const s=document.createElement('style');
    s.id='thorConsultaV082Style';
    s.textContent=`
      #thorConsultaGeralBtn{height:52px;white-space:nowrap;border:1px solid #d9dfdc;border-radius:12px;background:#fff;color:#38413c;padding:0 16px;font-weight:800;cursor:pointer}
      #thorConsultaGeralBtn:hover{border-color:#19834b;background:#f3fbf6;color:#176d41}
      .thorcg-modal{position:fixed;z-index:5000;inset:0;background:#1119;display:grid;place-items:center;padding:24px}
      .thorcg-card{width:min(1180px,96vw);max-height:92vh;background:#f7f9f8;border-radius:18px;box-shadow:0 28px 90px #0005;display:grid;grid-template-rows:auto auto minmax(0,1fr) auto;overflow:hidden}
      ...
      .thorcg-close{border:0;background:#eef2ef;width:40px;height:40px;border-radius:10px;font-size:22px;cursor:pointer}
      .thorcg-toolbar{display:flex;gap:10px;padding:14px 22px;background:#fff;border-bottom:1px solid #e3e8e5}
      .thorcg-toolbar input{height:46px;flex:1;border:1px solid #d6ddd9;border-radius:11px;padding:0 14px;font-size:16px}
      ...
      .thorcg-table{width:100%;border-collapse:collapse;min-width:920px}.thorcg-table th{background:#f4f7f5;text-align:left;padding:11px 12px;font-size:11px;text-transform:uppercase;color:#657069;border-bottom:1px solid #dfe5e1}
      .thorcg-table td{padding:12px;border-bottom:1px solid #edf1ee;font-size:14px}.thorcg-table tr:last-child td{border-bottom:0}.thorcg-name{font-weight:800}.thorcg-code{font-family:Consolas,monospace}.thorcg-price{font-weight:900;color:#176d41}
  ...
  function ensureButton(){
    ...
    const input=row?.querySelector('input.search');
    if(!row||!input||document.getElementById('thorConsultaGeralBtn'))return;
  ...
  async function open(){
    ...
    try{const r=await window.thor.allProducts();rows=Array.isArray(r)?r:[];apply();root.querySelector('#thorCgSearch').focus();}
