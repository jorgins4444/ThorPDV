(()=>{
  'use strict';
  if(window.__thorPaymentIconsV0913)return;
  window.__thorPaymentIconsV0913=true;
  ...
  const ICONS={
    ...
    store_credit:`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 7.5h17v11h-17z"/><path d="M7 11h5M7 15h3M15.5 10.5v4M13.5 12.5h4"/></svg>`,
    ...
    other:`<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="6" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="12" cy="12" r="1.3" fill="currentColor" stroke="none"/><circle cx="18" cy="12" r="1.3" fill="currentColor" stroke="none"/></svg>`
  };
  ...
  function detect(btn){
    ...
    if(text.includes('voucher')||text.includes('benefício')||text.includes('beneficio')) return 'voucher';
    if(text.includes('crédito da loja')||text.includes('credito da loja')||text.includes('vale crédito')||text.includes('vale credito')) return 'store_credit';
    if(text.includes('venda a prazo')||text.includes('parcel')) return 'installment';
    if(text.includes('cashback')) return 'cashback';
    if(text.includes('boleto')) return 'boleto';
    if(text.includes('pix')) return 'pix';
    if(text.includes('débito')||text.includes('debito')) return 'debit_card';
    if(text.includes('crédito')||text.includes('credito')) return 'credit_card';
    if(text.includes('dinheiro')||text.includes('espécie')||text.includes('especie')) return 'cash';
    return 'other';
  }
  ...
  function decorate(btn){
    ...
    if(icon.dataset.iconKey!==key){icon.innerHTML=ICONS[key]||ICONS.other;icon.dataset.iconKey=key;}
    btn.dataset.thorPaymentIcon='1';
  }

  function patch(){
    document.querySelectorAll('.v089-pay-methods,.payment-methods,[data-payment-methods],.v089-payment-methods')
      .forEach(root=>root.querySelectorAll('button').forEach(decorate));
  }
