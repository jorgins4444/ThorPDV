...
public class ThorReceiptRawPrinter {
 ...
 public static bool Send(string printer, byte[] bytes) {
   ...
   var di = new DOCINFOA { pDocName="ThorPDV Cupom Nao Fiscal", pDataType="RAW" };
...
'@; Add-Type -TypeDefinition $src -ErrorAction SilentlyContinue; $b=[Convert]::FromBase64String('${payload}'); if(-not [ThorReceiptRawPrinter]::Send('${printer}',$b)){throw 'raw_print_failed'}`;
}
...
function buildReceipt(sale, columns) {
  ...
  if (context.company_name) lines.push(center(context.company_name, width));
  if (context.branch_name && context.branch_name !== context.company_name) lines.push(center(context.branch_name, width));
  lines.push(center('COMPROVANTE NAO FISCAL', width));
  lines.push(strong);
  const saleNo = sale?.number || sale?.client_event_id || sale?.id || '';
  lines.push(leftRight('VENDA', fit(saleNo, Math.max(width - 8, 8)), width));
  lines.push(leftRight('DATA', new Date(sale?.completed_at || sale?.created_at || Date.now()).toLocaleString('pt-BR'), width));
  lines.push(sep);
  ...
  for (const item of sale?.items || []) {
    ...
    const firstWidth = Math.max(width - prefix.length, 8);
  ...
  lines.push(strong);
  const subtotal = Number(sale?.subtotal ?? 0) || (sale?.items || []).reduce((sum, item) => sum + Number(item?.quantity || 0) * Number(item?.unit_price ?? item?.unitPrice ?? 0), 0);
  const discount = Number(sale?.discount || 0);
  if (discount > 0) {
    lines.push(leftRight('SUBTOTAL', `R$ ${money(subtotal)}`, width));
    lines.push(leftRight('DESCONTO', `R$ ${money(discount)}`, width));
  }
  ...
  if (payments.length) {
    ...
    const labels = { cash:'DINHEIRO', pix:'PIX', debit_card:'DEBITO', credit_card:'CREDITO', voucher:'VOUCHER', store_credit:'CREDITO LOJA' };
    for (const p of payments) lines.push(leftRight(labels[p?.method] || String(p?.method || 'PAGAMENTO').toUpperCase(), `R$ ${money(p?.amount || 0)}`, width));
  }

  if (sale?.customer_name) {
    lines.push(sep);
    for (const line of wrap(`CLIENTE: ${sale.customer_name}`, width)) lines.push(line);
  }
...
function escposPayload(text, columns) {
...
async function printReceiptRaw(printerName, text, columns) {
  ...
  const payload = escposPayload(text, columns);
...
function installReceiptPrintingV829(ThorAgent, Store) {
  ...
  Store.prototype.saveSettings = function (input = {}) {
    if (Object.prototype.hasOwnProperty.call(input, 'receiptColumns')) {
      this.set('receipt_columns', Number(input.receiptColumns) === 65 ? '65' : '44');
    }
  ...
  ThorAgent.prototype.documentData = function (saleKey, type = 'pre_sale') {
    ...
    const text = buildReceipt(base.sale || {}, columns);
    return { ...base, kind:'text', text, html:htmlForReceipt(text, columns), receiptColumns:columns };
  };

  const originalPrintDocument = ThorAgent.prototype.printDocument;
  ThorAgent.prototype.printDocument = async function (saleKey, type = 'pre_sale') {
    if (type === 'nfce') return originalPrintDocument.call(this, saleKey, type);
    ...
    const columns = Number(this.settings()?.receiptColumns) === 65 ? 65 : 44;
    await printReceiptRaw(target, doc.text, columns);
...
module.exports = { installReceiptPrintingV829, buildReceipt, escposPayload };
