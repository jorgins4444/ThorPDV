...
def rep(path, old, new):
    ...
    if s.count(old)!=1: raise SystemExit(f'{path}: target mismatch: {old!r}')
    p.write_text(s.replace(old,new,1),encoding='utf-8')

rep('desktop-pdv/main.js',"const { installCommercialV070 } = require('./agent/commercial-v070');\n","const { installCommercialV070 } = require('./agent/commercial-v070');\nconst { installSalesOptionsV071 } = require('./agent/sales-options-v071');\n")
rep('desktop-pdv/main.js',"installCommercialV070(ThorAgent);\ninstallSyncPolicy(ThorAgent);","installCommercialV070(ThorAgent);\ninstallSalesOptionsV071(ThorAgent);\ninstallSyncPolicy(ThorAgent);")

p=Path('desktop-pdv/renderer/commercial-v070.js');s=p.read_text(encoding='utf-8')
start=s.index('  async function c70OpenTerm() {');end=s.index('\n  function c70OtherOptions()',start)
...
if old not in s:raise SystemExit('order card context target missing')
s=s.replace(old,old+"      v.salesOptionsOrderCard={brand:order.card_brand_code||'',acquirer:order.card_acquirer_cnpj||'',installments:Number(order.card_installments||1)};\n",1)
old="  v3ResetSale=function(){const result=previousReset();const c=c70State();c.salesOrderId=null;c.salesOrderNumber=null;c.orderPriceLock=false;c.term=null;c.preferredPaymentMethod=null;return result;};"
if old not in s:raise SystemExit('reset target missing')
s=s.replace(old,"  v3ResetSale=function(){const result=previousReset();const c=c70State(),v=v3State();c.salesOrderId=null;c.salesOrderNumber=null;c.orderPriceLock=false;c.term=null;c.preferredPaymentMethod=null;v.salesOptionsOrderCard=null;return result;};",1)
p.write_text(s,encoding='utf-8')

rep('desktop-pdv/renderer/index.html','<link rel="stylesheet" href="commercial-v070.css"></head>','<link rel="stylesheet" href="commercial-v070.css"><link rel="stylesheet" href="sales-options-v071.css"></head>')
rep('desktop-pdv/renderer/index.html','<script src="commercial-v070.js"></script></body>','<script src="commercial-v070.js"></script><script src="sales-options-v071.js"></script></body>')
