...
def rep(path: str, old: str, new: str):
    p = Path(path)
    s = p.read_text(encoding='utf-8')
    c = s.count(old)
    if c != 1:
        raise SystemExit(f'{path}: expected one match, got {c}: {old!r}')
...
p = Path('desktop-pdv/renderer/commercial-v070.js')
s = p.read_text(encoding='utf-8')
start = s.index('  async function c70OpenTerm() {')
end = s.index('\n  function c70OtherOptions()', start)
new_block = r'''  async function c70OpenTerm() {
...
old = "      c.salesOrderId=order.id;c.salesOrderNumber=order.number;c.orderPriceLock=true;c.preferredPaymentMethod=order.payment_condition==='immediate'?order.payment_method:null;\n"
new = old + "      v.salesOptionsOrderCard={brand:order.card_brand_code||'',acquirer:order.card_acquirer_cnpj||'',installments:Number(order.card_installments||1)};\n"
if old not in s:
    raise SystemExit('commercial order context target not found')
s = s.replace(old, new, 1)
old_reset = "  v3ResetSale=function(){const result=previousReset();const c=c70State();c.salesOrderId=null;c.salesOrderNumber=null;c.orderPriceLock=false;c.term=null;c.preferredPaymentMethod=null;return result;};"
new_reset = "  v3ResetSale=function(){const result=previousReset();const c=c70State(),v=v3State();c.salesOrderId=null;c.salesOrderNumber=null;c.orderPriceLock=false;c.term=null;c.preferredPaymentMethod=null;v.salesOptionsOrderCard=null;return result;};"
if old_reset not in s:
    raise SystemExit('commercial reset target not found')
s = s.replace(old_reset, new_reset, 1)
p.write_text(s, encoding='utf-8')

rep(
    'desktop-pdv/renderer/index.html',
    '<link rel="stylesheet" href="commercial-v070.css"></head>',
    '<link rel="stylesheet" href="commercial-v070.css"><link rel="stylesheet" href="sales-options-v071.css"></head>',
)
rep(
    'desktop-pdv/renderer/index.html',
    '<script src="commercial-v070.js"></script></body>',
    '<script src="commercial-v070.js"></script><script src="sales-options-v071.js"></script></body>',
)
rep('desktop-pdv/package.json', '"version": "0.7.0"', '"version": "0.7.1"')

p = Path('.github/workflows/pdv-desktop.yml')
s = p.read_text(encoding='utf-8')
old = "            'agent/pre-sale-v064.js',\n"
if old not in s:
    raise SystemExit('agent validation insertion target not found')
s = s.replace(old, old + "            'agent/sales-options-v071.js',\n", 1)
old = "            'renderer/quick-entry-v063.js'\n"
if old not in s:
    raise SystemExit('renderer validation insertion target not found')
s = s.replace(old, "            'renderer/quick-entry-v063.js',\n            'renderer/commercial-v070.js',\n            'renderer/sales-options-v071.js'\n", 1)
p.write_text(s, encoding='utf-8')
