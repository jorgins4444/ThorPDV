...
export async function erpProductList(search?:string){const token=await getSessionToken();const result=await rpc('erp_product_list',{p_token:token,p_search:search?.trim()||null});return {ok:Boolean(result.ok),error:result.error,data:Array.isArray(result.data)?result.data:[],branch_id:result.branch_id};}
...
export async function erpProductSave(payload:Record<string,unknown>){const token=await getSessionToken();return rpc('erp_product_save_v3',{p_token:token,p_payload:payload});}
