...
export function SalesOptionsWorkspace({initial}:{initial:SalesOptions}){
  ...
  const activeAcquirers=useMemo(()=>acquirers.filter(a=>a.active!==false),[acquirers]);
  const defaultAcquirer=useMemo(()=>activeAcquirers.find(a=>Boolean(a.preferred))||activeAcquirers[0]||null,[activeAcquirers]);
  async function routeBrand(row:Row,cnpj:string){const code=s(row.code);setBrands(rows=>rows.map(x=>s(x.code)===code?{...x,acquirer_cnpj:cnpj,acquirer_name:s(activeAcquirers.find(a=>s(a.cnpj)===cnpj)?.name)}:x));const label=cnpj?s(activeAcquirers.find(a=>s(a.cnpj)===cnpj)?.name):'padrão da filial';await run(`brand-acq:${code}`,()=>salesCardBrandAcquirerSave({code,acquirer_cnpj:cnpj}),`${s(row.name)} direcionada para ${label}.`);}
  ...
  async function preferAcquirer(row:Row){const cnpj=s(row.cnpj);setAcquirers(rows=>rows.map(x=>({...x,active:s(x.cnpj)===cnpj?true:x.active,preferred:s(x.cnpj)===cnpj})));await run(`pref:${cnpj}`,()=>salesCardAcquirerSave({cnpj,active:true,preferred:true}),`${s(row.name)} definida como padrão da filial.`);}
  ...
  return <div className="erp-sales-options-shell">
    ...
    <section className="erp-module-card erp-sales-options-section">
      <div className="erp-sales-options-head"><div><small>CARTÕES</small><h2>Bandeiras e roteamento por credenciadora</h2><p>Ative as bandeiras aceitas e escolha, dentro de cada bandeira, qual credenciadora deve ser usada nesta filial. Você pode usar credenciadoras diferentes por bandeira.</p></div></div>
      <div className="erp-brand-routing-grid">{brands.map(row=>{const code=s(row.code),active=row.active!==false,current=s(row.acquirer_cnpj);return <article className={`erp-brand-route ${active?'on':'off'}`} key={code}><div className="erp-brand-route-head"><div><b>{s(row.name)}</b><small>{active?'Bandeira ativa':'Bandeira desativada'}</small></div><label className="erp-switch"><input type="checkbox" checked={active} disabled={busy===`brand:${code}`} onChange={()=>toggleBrand(row)}/><span/></label></div><label><span>Credenciadora desta bandeira</span><select value={current} disabled={!active||busy===`brand-acq:${code}`} onChange={e=>void routeBrand(row,e.target.value)}><option value="">Padrão da filial{defaultAcquirer?` — ${s(defaultAcquirer.name)}`:''}</option>{activeAcquirers.map(a=><option key={s(a.cnpj)} value={s(a.cnpj)}>{s(a.name)}</option>)}</select></label><div className="erp-brand-route-foot"><span>{current?'Roteamento específico':'Usando roteamento padrão'}</span>{current&&<strong>{s(row.acquirer_name)||s(activeAcquirers.find(a=>s(a.cnpj)===current)?.name)}</strong>}</div></article>})}</div>
      {!activeAcquirers.length&&<div className="erp-sales-routing-warning">Habilite ao menos uma credenciadora na seção abaixo para poder vinculá-la às bandeiras.</div>}
    </section>

    <section className="erp-module-card erp-sales-options-section">
      <div className="erp-sales-options-head"><div><small>CARTÕES</small><h2>Credenciadoras disponíveis</h2><p>Habilite aqui as adquirentes com as quais a filial trabalha. A marcada como padrão será usada somente quando uma bandeira não tiver credenciadora específica.</p></div><div className="erp-acquirer-filters"><input value={acquirerSearch} onChange={e=>setAcquirerSearch(e.target.value)} placeholder="Buscar por nome ou CNPJ"/><label><input type="checkbox" checked={showEnabledAcquirers} onChange={e=>setShowEnabledAcquirers(e.target.checked)}/> Somente habilitadas</label></div></div>
      <div className="erp-acquirer-list">{filteredAcquirers.map(row=>{const cnpj=s(row.cnpj),active=row.active!==false,preferred=Boolean(row.preferred);return <article className={`erp-acquirer-row ${active?'enabled':''}`} key={cnpj}><label className="erp-switch"><input type="checkbox" checked={active} onChange={()=>toggleAcquirer(row)}/><span/></label><div><strong>{s(row.name)}</strong><small>CNPJ {cnpj} · Status {s(row.status||'Ativo')}</small></div><button type="button" className={`erp-preferred ${preferred?'active':''}`} disabled={!active||busy===`pref:${cnpj}`} onClick={()=>preferAcquirer(row)}>{preferred?'★ Padrão da filial':'☆ Usar como padrão'}</button></article>})}{!filteredAcquirers.length&&<div className="erp-empty">Nenhuma credenciadora encontrada para o filtro.</div>}</div>
    </section>

    <section className="erp-module-card erp-sales-options-section">
      <div className="erp-sales-options-head"><div><small>CARTÃO DE CRÉDITO</small><h2>Parcelamento de 1x a 12x</h2><p>Estas são as opções que aparecem na finalização do ThorPDV. No Crédito, o operador escolhe somente a quantidade de parcelas; a credenciadora fica administrada no ThorGestão.</p></div></div>
