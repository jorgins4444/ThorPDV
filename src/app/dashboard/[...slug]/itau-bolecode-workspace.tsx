...
const err:Record<string,string>={
 ...
 itau_production_integration_not_found:'Esta conta não está vinculada a uma integração Itaú de Produção.',
 ...
 production_credentials_incomplete:'A Produção ainda não está pronta no ThorControl. Configure Client ID, Client Secret, certificado, chave privada e URL produtiva.',
 production_api_url_missing:'A URL produtiva da API BoleCode ainda não foi configurada.',
 ...
 itau_production_token_failed:'O Itaú recusou o token OAuth de Produção via mTLS.',
 itau_production_mtls_exception:'Falha na conexão mTLS com o Itaú. Verifique certificado e chave privada.',
 itau_production_edge_failed:'A camada segura de Produção não respondeu corretamente.',
...
export function ItauBolecodeWorkspace({accountsInitial,integrationsInitial,billingsInitial}:{accountsInitial:Record<string,unknown>;integrationsInitial:Record<string,unknown>;billingsInitial:Record<string,unknown>}){
 ...
 async function documentationExampleTest(){
  if(!integration||!isSandbox)return;
 ...
 async function customTest(e:FormEvent<HTMLFormElement>){
  ...
  if(effective&&!isSandbox&&!window.confirm('ATENÇÃO: esta operação emitirá uma cobrança REAL no Itaú usando a conta beneficiária configurada. Deseja continuar?'))return;
  ...
  const okText=isSandbox?(effective?'Payload personalizado enviado ao Sandbox Itaú.':'Simulação personalizada enviada ao Sandbox Itaú.'):(effective?'Cobrança REAL enviada ao Itaú em Produção.':'Simulação enviada ao Itaú em Produção sem efetivação.');
  applyResult(r as Row,okText);
  await refresh();
 }

 return <div className="itau-bank-workspace">
  <section className="itau-hero">
   <div><small>{isSandbox?'CENTRAL DE HOMOLOGAÇÃO':'OPERAÇÃO PRODUTIVA'}</small><h2>Itaú BoleCode Pix</h2><p>{isSandbox?'Valide credenciais, comunicação com o gateway e acompanhe os retornos reais do Sandbox.':'Produção usa OAuth + mTLS. Simule antes de efetivar e acompanhe o retorno bancário de cada cobrança.'}</p></div>
   <div className="itau-state"><span className={integration?.active&&integration?.provider_ready?'on':'off'}/><div><strong>{integration?.active&&integration?.provider_ready?(isSandbox?'OAuth pronto para chamadas':'mTLS de Produção pronto'):integration?'Integração requer atenção':'Nenhuma conta vinculada'}</strong><small>{integration?`${isSandbox?'Sandbox':'Produção'} · ${integration.provider_ready?'Credenciais validadas':'Credencial/certificado pendente no ThorControl'}`:'Configure boletos dentro de Contas Bancárias'}</small></div></div>
  </section>
  ...
  {configured.length===0?<section className="itau-card itau-empty-state"><div className="bank-provider-logo">Itaú</div><div><small>AINDA NÃO VINCULADO</small><h3>Configure o BoleCode dentro da conta bancária</h3><p>Volte para Contas Bancárias, abra uma conta Itaú e use <b>Configurar boletos bancários</b>.</p></div><a className="itau-primary" href="/dashboard/financeiro/contas-bancarias">Ir para Contas Bancárias</a></section>:<>
   <section className="itau-account-picker">
    <label>Integração<select value={text(integration?.id)} onChange={e=>{setSelectedIntegrationId(e.target.value);setResult(null);setMessage('')}}>{configured.map(i=><option key={text(i.id)} value={text(i.id)}>{text(i.account_name)} · {text(i.environment)==='production'?'Produção':'Sandbox'}</option>)}</select></label>
   ...
   <section className="itau-card itau-linked-summary">
    <div className="itau-card-head"><div><small>VÍNCULO ATUAL</small><h3>{text(selected?.name)||text(integration?.account_name)}</h3></div><span className={`itau-pill ${integration?.provider_ready?'ready':''}`}>{integration?.provider_ready?(isSandbox?'Credencial ativa':'mTLS pronto'):'Configuração pendente'}</span></div>
    ...
    <div className="itau-linked-actions"><a href="/dashboard/financeiro/contas-bancarias">Editar na conta bancária</a><span>As credenciais e o certificado permanecem centralizados no ThorControl.</span></div>
   </section>

   <details className="itau-card itau-simulator" open={!isSandbox}>
    <summary><div><small>{isSandbox?'LABORATÓRIO SANDBOX':'BOLECODE EM PRODUÇÃO'}</small><h3>{isSandbox?'Simulador BoleCode':'Cobrança BoleCode'}</h3><p>{isSandbox?'O Sandbox usa dados fictícios e cenários internos do Itaú.':'Use Simular para validar os dados sem efetivar. O botão de emissão exige confirmação antes de gerar cobrança real.'}</p></div><span>{isSandbox?'Abrir simulador ↓':'Operação produtiva ↓'}</span></summary>
    <div className="itau-test">
     {isSandbox?<section className="itau-official-scenario">
      ...
      <div className="itau-official-action"><div><b>O que este teste verifica?</b><span>Obtenção do token, acesso ao endpoint, autorização do gateway, envio do JSON e interpretação da resposta.</span></div><button type="button" className="itau-primary" onClick={()=>run(documentationExampleTest)} disabled={pending||!integration?.provider_ready||!integration?.active}>{pending?'Processando...':'Executar exemplo da documentação'}</button></div>
     </section>:null}

     <details className="itau-custom-scenario" open={!isSandbox}>
      <summary><div><b>{isSandbox?'Payload personalizado':'Dados da cobrança'}</b><span>{isSandbox?'Avançado · depende de cenário disponível no Sandbox':'Produção · conta e beneficiário reais'}</span></div><strong>Abrir ↓</strong></summary>
      <form onSubmit={e=>run(()=>customTest(e))}>
       <div className="itau-custom-warning"><b>{isSandbox?'Atenção ao Sandbox':'Atenção — ambiente real'}</b><span>{isSandbox?'Dados livres podem receber HTTP 500 “Cenário de teste não mapeado”. Esse retorno indica limitação do mock, não falha de autenticação.':'Simular executa as validações sem efetivar. Emitir cobrança REAL registra o BoleCode no Itaú e exige confirmação.'}</span></div>
       <div className="itau-form-grid"><label className="wide">Nome / Razão social<input required name="name" placeholder="Cliente"/></label><label>CPF/CNPJ<input required name="document" inputMode="numeric" placeholder="Somente números"/></label><label>Valor<input required name="amount" type="number" min="0.01" step="0.01" placeholder="100.00"/></label><label>Vencimento<input required name="due_date" type="date"/></label><label>CEP<input required name="postal_code" inputMode="numeric" maxLength={9}/></label><label className="wide">Logradouro<input required name="street" placeholder="Rua / Avenida"/></label><label>Número<input name="number" placeholder="100"/></label><label>Complemento<input name="complement"/></label><label>Bairro<input required name="district"/></label><label>Cidade<input required name="city"/></label><label>UF<input required name="state" maxLength={2} placeholder="PI"/></label></div>
       <div className="itau-test-actions"><button value="simulate" disabled={pending||!integration?.provider_ready||!integration?.active}>{pending?'Processando...':'Simular no Itaú'}</button><button className="itau-primary" value="effective" disabled={pending||!integration?.provider_ready||!integration?.active}>{isSandbox?'Emitir teste Sandbox':'Emitir cobrança REAL'}</button></div>
      </form>
     </details>
    </div>
   </details>
  ...
  {result?<section className="itau-card itau-result">
   ...
   {!isSandbox&&result.ok?<div className="itau-result-context"><b>Produção Itaú</b><span>Retorno recebido pela camada mTLS. Confira status, Nosso Número, linha digitável e Pix antes de compartilhar a cobrança.</span></div>:null}
