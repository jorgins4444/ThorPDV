...
type SupplierXml={document:string;name:string;trade_name:string;state_registration:string;email:string;phone:string;street:string;number:string;complement:string;district:string;city:string;state:string;postal_code:string;ibge_city_code:string};
...
type ParsedNfe={key:string;model:string;series:string;number:string;issueDate:string;supplier:SupplierXml;items:ParsedItem[];installments:Installment[];totals:{products:number;discount:number;freight:number;insurance:number;other:number;invoice:number;taxAdjustment:number;icms:number;ipi:number;st:number};rawXml:string};
...
function parseNfe(rawXml:string):ParsedNfe{
  ...
  const ide=xmlEl(inf,'ide');const emit=xmlEl(inf,'emit');const ender=emit?xmlEl(emit,'enderEmit'):null;const total=xmlEl(inf,'ICMSTot');
  ...
  const supplier:SupplierXml={
    document:digits(xmlText(emit,'CNPJ')||xmlText(emit,'CPF')),name:xmlText(emit,'xNome'),trade_name:xmlText(emit,'xFant'),state_registration:xmlText(emit,'IE'),
    email:xmlText(emit,'email'),phone:xmlText(ender,'fone'),street:xmlText(ender,'xLgr'),number:xmlText(ender,'nro'),complement:xmlText(ender,'xCpl'),district:xmlText(ender,'xBairro'),city:xmlText(ender,'xMun'),state:xmlText(ender,'UF'),postal_code:digits(xmlText(ender,'CEP')),ibge_city_code:xmlText(ender,'cMun'),
  };
...
export function PurchaseXmlWorkspace({suppliers,products,links,units,categories,chartAccounts,costCenters,currentBranchId,currentBranchName}:{suppliers:Row[];products:Row[];links:Row[];units:Row[];categories:Row[];chartAccounts:Row[];costCenters:Row[];currentBranchId:string;currentBranchName:string}){
  ...
  const ready=Boolean(nfe)&&Boolean(supplierId)&&itemsReady&&financeReady;
  const matchedCount=items.filter(i=>!i.createProduct&&i.productId).length;const newCount=items.filter(i=>i.createProduct).length;
  ...
  async function finish(){
    ...
    const r=await purchaseXmlImport({
      supplier_id:supplierId==='__new__'?null:supplierId,supplier:nfe.supplier,nfe_access_key:nfe.key,nfe_model:nfe.model,nfe_series:nfe.series,document_number:nfe.number,issue_date:nfe.issueDate,due_date:installments[0]?.due_date,
      ...
      xml_metadata:{file_name:fileName,raw_xml:nfe.rawXml,totals:nfe.totals,supplier:nfe.supplier,imported_from:'thorgestao_purchase_xml'},
    });
    setSaving(false);if(!r.ok){setMessage(`Não foi possível importar a NF-e: ${importError(r.error)}`);return;}
    setMessage(`NF-e ${nfe.number} importada com sucesso como compra nº ${text(r.number)}. Estoque, custos, preços e ${text(r.installments)} título(s) a pagar foram atualizados.`);setNfe(null);setItems([]);setInstallments([]);setFileName('');setSupplierId('');
  }

  return <div className="pxml">
    <section className="pxml-progress">
      {['1 · XML','2 · Fornecedor','3 · Produtos e conversões','4 · Financeiro','5 · Confirmar entrada'].map((label,index)=>{const ok=index===0?Boolean(nfe):index===1?Boolean(nfe&&supplierId):index===2?itemsReady:index===3?financeReady:ready;return <div key={label} className={ok?'done':''}><span>{ok?'✓':index+1}</span><b>{label.replace(/^\d · /,'')}</b></div>})}
    </section>

    <section className="pxml-card pxml-upload">
      <div><span>IMPORTAÇÃO DE NF-e</span><h2>Carregar XML da nota fiscal</h2><p>O arquivo é lido para conferência antes de qualquer alteração de estoque ou financeiro.</p></div>
      <div className="pxml-upload-actions"><label className="pxml-file">Selecionar XML<input type="file" accept=".xml,text/xml,application/xml" onChange={loadXml}/></label><Link href="/dashboard/compras" className="pxml-secondary">Entrada manual</Link></div>
      {fileName&&<small>Arquivo carregado: <b>{fileName}</b></small>}
    </section>

    {message&&<div className="pxml-message">{message}</div>}

    {nfe&&<>
      <section className="pxml-card"><header className="pxml-title"><div><span>NF-e IDENTIFICADA</span><h3>Nota {nfe.number} · Série {nfe.series||'—'}</h3><p>Emissão {fmtDate(nfe.issueDate)} · modelo {nfe.model} · chave {nfe.key}</p></div><span className="pxml-ok">XML lido</span></header><div className="pxml-kpis"><div><span>Produtos</span><b>{money(nfe.totals.products)}</b></div><div><span>Desconto</span><b>{money(nfe.totals.discount)}</b></div><div><span>Frete + seguro + outros</span><b>{money(nfe.totals.freight+nfe.totals.insurance+nfe.totals.other)}</b></div><div><span>Total NF-e</span><strong>{money(nfe.totals.invoice)}</strong></div></div><div className="pxml-taxline"><span>ICMS informado: {money(nfe.totals.icms)}</span><span>IPI: {money(nfe.totals.ipi)}</span><span>ST: {money(nfe.totals.st)}</span><span>Ajuste fiscal que compõe vNF: {money(nfe.totals.taxAdjustment)}</span></div></section>

      <section className="pxml-card"><header className="pxml-title"><div><span>FORNECEDOR</span><h3>{nfe.supplier.name}</h3><p>CNPJ/CPF {nfe.supplier.document||'—'} · IE {nfe.supplier.state_registration||'—'} · {nfe.supplier.city}/{nfe.supplier.state}</p></div></header><div className="pxml-form-grid"><label className="wide">Fornecedor no ThorGestão<select value={supplierId} onChange={e=>changeSupplier(e.target.value)}><option value="__new__">＋ Cadastrar fornecedor com os dados do XML</option>{suppliers.map(s=><option key={text(s.id)} value={text(s.id)}>{text(s.name)} · {text(s.document)}</option>)}</select><small>{supplierId==='__new__'?'Será cadastrado somente quando a entrada for confirmada.':'Fornecedor existente reconhecido/selecionado.'}</small></label></div></section>

      <section className="pxml-card"><header className="pxml-title"><div><span>ITENS DA NOTA</span><h3>Vínculo, conversão e preço de venda</h3><p>Exemplo: XML em CX com 10 caixas × fator 12 = entrada de 120 UN. O custo é recalculado por unidade de estoque.</p></div><div className="pxml-badges"><span>{matchedCount} vinculados</span><span>{newCount} novos</span></div></header><div className="pxml-table-wrap"><table><thead><tr><th>Item do XML</th><th>Produto ThorGestão</th><th>Conversão</th><th>Entrada no estoque</th><th>Custo unit.</th><th>Preço venda</th><th>Margem</th></tr></thead><tbody>{items.map((item,index)=>{const cost=unitCost(item);const margin=cost>0?(item.salePrice/cost-1)*100:0;return <tr key={`${item.itemNo}-${item.code}`} className={item.createProduct?'new-product':''}><td><b>{item.code} · {item.name}</b><small>GTIN {item.ean||'sem GTIN'} · NCM {item.ncm||'—'} · CFOP {item.cfop||'—'}</small><small>XML: {item.quantity} {item.unit} × {money(item.unitPrice)} · líquido {money(item.netTotal)}</small></td><td><select value={item.createProduct?'__new__':item.productId} onChange={e=>chooseProduct(index,e.target.value)}><option value="__new__">＋ Cadastrar novo produto</option>{products.map(p=><option key={text(p.id)} value={text(p.id)}>{text(p.product_code)||text(p.sku)} · {text(p.name)} · {text(p.unit)}</option>)}</select><small>{item.matchReason}</small>{item.createProduct&&<label className="inline">Unidade estoque<select value={item.stockUnit} onChange={e=>patchItem(index,{stockUnit:e.target.value})}>{units.map(u=><option key={text(u.code)} value={text(u.code)}>{text(u.code)} · {text(u.name)}</option>)}</select></label>}</td><td><label>1 {item.unit} = <input type="number" min="0.000001" step="0.001" value={item.conversionFactor} onChange={e=>patchItem(index,{conversionFactor:Math.max(num(e.target.value),0)})}/> {item.stockUnit}</label>{item.unit.toUpperCase()!==item.stockUnit.toUpperCase()&&<small className="attention">Confira o fator de conversão.</small>}</td><td><b>{stockQty(item).toLocaleString('pt-BR')} {item.stockUnit}</b><small>Quantidade que será somada ao estoque</small></td><td><b>{money(cost)}</b><small>Custo líquido após conversão</small></td><td><input className="price" type="number" min="0.01" step="0.01" value={item.salePrice} onChange={e=>patchItem(index,{salePrice:num(e.target.value)})}/><small>{item.createProduct?'Preço inicial do novo produto':'Novo preço de venda'}</small></td><td><b className={margin<0?'negative':''}>{margin.toLocaleString('pt-BR',{maximumFractionDigits:2})}%</b></td></tr>})}</tbody></table></div></section>
      ...
      <section className="pxml-card pxml-confirm"><div><span>CONFIRMAÇÃO FINAL</span><h3>Importar NF-e e efetivar a entrada</h3><p>Ao confirmar, o ThorGestão grava a chave da NF-e, atualiza estoque/custo/preço, memoriza as conversões do fornecedor e cria as contas a pagar.</p><ul><li>Fornecedor: {supplierId==='__new__'?'será cadastrado pelo XML':'vinculado ao cadastro existente'}</li><li>Itens: {items.length} · {matchedCount} vinculados · {newCount} novos</li><li>Total NF-e: {money(nfe.totals.invoice)} · Financeiro: {money(financialTotal)}</li></ul></div><button className="pxml-import" disabled={!ready||saving} onClick={()=>void finish()}>{saving?'Importando NF-e...':'✓ Importar nota fiscal'}</button></section>
    </>}
  </div>;
}
