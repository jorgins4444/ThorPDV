...
type RpcResult={ok?:boolean;error?:string;data?:Row[];session?:Row;payments?:Row[];movements?:Row[];sales?:Row[];returns?:Row[];audit?:Row[];fiscal?:Row;snapshot?:Row;can_correct?:boolean;can_reopen?:boolean;permission?:string;[key:string]:unknown};
...
export async function cashClosureHistory(filters:{start?:string;end?:string;operatorId?:string;branchId?:string}={}){
  ...
  const r=await rpc('erp_cash_closure_history_v2',{p_token,p_start:filters.start||null,p_end:filters.end||null,p_operator:filters.operatorId||null,p_branch:filters.branchId||null});
...
export async function cashClosureDetail(cashId:string,closureAuditId?:string){
  const p_token=await token();
  const r=await rpc('erp_cash_closure_detail_v3',{p_token,p_cash_id:cashId,p_closure_audit_id:closureAuditId||null});
  return {
    ok:Boolean(r.ok),error:r.error,
    session:(r.session&&typeof r.session==='object'?r.session:{}) as Row,
    payments:Array.isArray(r.payments)?r.payments:[],movements:Array.isArray(r.movements)?r.movements:[],sales:Array.isArray(r.sales)?r.sales:[],returns:Array.isArray(r.returns)?r.returns:[],audit:Array.isArray(r.audit)?r.audit:[],
