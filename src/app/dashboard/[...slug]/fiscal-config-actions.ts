...
export async function nfeSaleDraftCreate(saleId:string,seriesId:string|undefined,operation:Record<string,unknown>){
  const result=await rpc('erp_nfe_sale_draft_create',{p_token:await token(),p_sale_id:saleId,p_series_id:seriesId||null,p_operation:operation});
  if(result.error==='fiscal_preflight_failed'||result.error==='fiscal_configuration_incomplete'){
    const validation=Array.isArray(result.validation_errors)?result.validation_errors.map(v=>String(v)).filter(Boolean):[];
    return {...result,error_code:String(result.error),error:validation.length?validation.join(' · '):'Revise a operação e os CFOPs antes de preparar a NF-e.'};
  }
  return result;
}
export async function nfeManualDraftCreate(payload:Record<string,unknown>){
  const result=await rpc('erp_nfe_manual_draft_create_v2',{p_token:await token(),p_payload:payload});
  if(result.error==='fiscal_preflight_failed'||result.error==='fiscal_configuration_incomplete'){
    const validation=Array.isArray(result.validation_errors)?result.validation_errors.map(v=>String(v)).filter(Boolean):[];
