...
export function BankHomologationWorkspace({initial}:{initial:Row}){
  ...
  async function generateTest(){
    ...
    download(text(r.file_name)||'HOMOLOGACAO.REM',text(r.content));setMessage(`Remessa teste gerada. Nosso Número ${text(b.our_number)}. Envie este arquivo no ambiente de teste/homologação do banco.`);await refresh(selectedConfig);setViewStep(5);
  }
  ...
  return <div className="hom-studio">
    ...
    <main className="hom-stage">
      ...
      {viewStep===5?<div className="hom-stage-card"><header><span>ETAPA 5</span><h2>Gerar e enviar a remessa teste</h2><p>O arquivo é gerado pelo mesmo motor que será usado em produção, porém a trava de homologação permite somente o título reservado.</p></header>{chosenTest?<div className="hom-test-title"><div><small>TÍTULO TESTE</small><b>{text(chosenTest.customer)}</b><span>{text(chosenTest.description)} · {date(chosenTest.due_date)}</span></div><strong>{money(chosenTest.remaining)}</strong></div>:null}{hom?.test_remittance_id?<div className="hom-file-ready"><span>ARQUIVO GERADO</span><b>{text((data.remittances as Row[]|undefined)?.find(r=>text(r.id)===text(hom.test_remittance_id))?.file_name)||'Remessa de homologação'}</b><small>Gerado em {dateTime(hom.remittance_generated_at)}</small><div><button type="button" onClick={()=>void redownloadTest()} disabled={busy}>↓ Baixar novamente</button>{text(hom.status)==='test_generated'?<button className="primary" type="button" onClick={()=>void markSent()} disabled={busy}>Marcar como enviada ao banco</button>:<span className="hom-wait">Enviada · aguardando retorno</span>}</div></div>:<div className="hom-generate-box"><b>Pronto para gerar</b><span>Será criado um arquivo contendo somente o título selecionado.</span><button className="primary" type="button" onClick={()=>void generateTest()} disabled={busy}>{busy?'Gerando...':`Gerar remessa teste ${layoutLabel(activeLayout)}`}</button></div>}<div className="hom-actions"><button type="button" onClick={()=>setViewStep(4)}>Voltar</button>{text(hom?.status)==='awaiting_return'?<button className="primary" type="button" onClick={()=>setViewStep(6)}>Importar retorno</button>:null}</div></div>:null}
