...
export function PayablesWorkspaceV2({initial,suppliers,accounts,paymentMethods,categories,chartAccounts,costCenters}:{initial:Row[];suppliers:Row[];accounts:Row[];paymentMethods:Row[];categories:Row[];chartAccounts:Row[];costCenters:Row[]}){
  ...
  const postingAccounts=useMemo(()=>managerialAccounts.filter(a=>a.active!==false&&a.posting!==false),[managerialAccounts]);
  const activeCenters=useMemo(()=>centers.filter(c=>c.active!==false),[centers]);
  ...
  async function createSubmit(e:FormEvent<HTMLFormElement>){
    ...
    await refresh();setModal(null);setMessage('Conta a pagar lançada e classificada com sucesso.');
  }
  ...
  async function classifySubmit(e:FormEvent<HTMLFormElement>){
    ...
    await refresh();setModal(null);setSelected(null);setMessage('Classificação gerencial atualizada.');
  }
  ...
  return <div className="payables-v2">
    ...
    <section className="payables-card">
      ...
      <div className="payables-table-wrap"><table className="classified"><thead><tr><th>Vencimento</th><th>Fornecedor</th><th>Descrição</th><th>Classificação</th><th>Centro de custo</th><th>Valor</th><th>Saldo</th><th>Situação</th><th></th></tr></thead><tbody>
