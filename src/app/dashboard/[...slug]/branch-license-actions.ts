...
type RpcResult={ok?:boolean;error?:string;data?:Record<string,unknown>[];[key:string]:unknown};
...
export async function branchLicenseList(){
  const t=await token();
  const result=await rpc('erp_branch_list',{p_token:t});
...
export async function branchLicenseSave(payload:Record<string,unknown>){
  const t=await token();
  const result=await rpc('erp_branch_profile_save',{p_token:t,p_payload:payload});
  if(result.ok){
