...
export function BankCnabMultiWorkspace({initial}:{initial:Row}){
  ...
  async function saveConfig(e:FormEvent<HTMLFormElement>){
    ...
    setConfigOpen(false);setMessage(`${layoutLabel(layout)} Itaú configurado. Conclua a Homologação Bancária antes da primeira remessa produtiva.`);await refresh(text(r.config_id));
  }
  ...
  async function generate(){
    ...
    setMessage(`${layoutLabel(activeLayout)}: remessa ${text(r.file_name)} gerada com ${text(r.record_count)} título(s), total ${money(r.total_amount)}. Os boletos estão disponíveis na opção “Boletos” desta remessa.`);await refresh();
  }
  ...
  return <div className="cnab-studio">
    ...
    <section className="cnab-flow"><div><i>1</i><b>Configurar</b><span>Layout, agência, conta e carteira</span></div><div><i>2</i><b>Gerar .REM</b><span>Selecionar títulos em aberto</span></div><div><i>3</i><b>Imprimir boletos</b><span>Padrão bancário Itaú A4</span></div><div><i>4</i><b>Enviar ao Itaú</b><span>Transmissão de arquivos</span></div><div><i>5</i><b>Importar .RET</b><span>Baixa e conciliação automáticas</span></div></section>
