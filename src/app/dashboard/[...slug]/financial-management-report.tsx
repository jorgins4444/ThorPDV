...
export function FinancialManagementReport({report,branches,initial}:{report:Kind;branches:Row[];initial:ReportResult}){
  ...
  async function load(){setLoading(true);setError('');const r=await erpReportV2(report,start,end,branch||undefined);setLoading(false);if(!r.ok){setError(text(r.error||'Erro ao carregar relatório'));return}setData(Array.isArray(r.data)?r.data:[]);}
