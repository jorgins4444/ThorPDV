-- ThorFiscal: materializacao segura para assinatura/transmissao e status A1 no PDV.
...
begin
  ...
  if v_doc.document_type <> 'nfce' then
    return jsonb_build_object('ok',false,'error','nfce_only');
  end if;
  ...
  if v_doc.status = 'authorized' then
    return jsonb_build_object(
      ...
      'document',jsonb_build_object(
        'id',v_doc.id,'status',v_doc.status,'access_key',v_doc.access_key,
        'protocol',v_doc.protocol,'authorization_at',v_doc.authorization_at,
        'number',v_doc.number,'series',v_doc.series
      )
    );
  end if;

  if v_doc.status = 'cancelled' then
    return jsonb_build_object('ok',false,'error','document_cancelled');
  end if;

  if v_doc.sale_id is null then
    return jsonb_build_object('ok',false,'error','sale_not_linked');
  end if;

  select * into v_sale
  from public.sales
  where id=v_doc.sale_id and tenant_id=v_doc.tenant_id and company_id=v_doc.company_id;
  ...
  select * into v_cert
  from private.fiscal_certificates
  where tenant_id=v_doc.tenant_id and company_id=v_doc.company_id;

  if v_cert.company_id is null then
    return jsonb_build_object('ok',false,'error','certificate_not_configured');
  end if;
  if v_cert.valid_to is not null and v_cert.valid_to <= now() then
    return jsonb_build_object('ok',false,'error','certificate_expired','valid_to',v_cert.valid_to);
  end if;
  ...
  if v_doc.number is null or v_doc.number !~ '^[0-9]+$' then
    ...
    update public.fiscal_documents
      set series=v_series,number=v_number::text,updated_at=now()
      where id=v_doc.id
  ...
  if nullif(v_company.cnpj,'') is null
     or nullif(v_company.state_registration,'') is null
     or v_company.tax_regime is null then
    return jsonb_build_object('ok',false,'error','company_fiscal_data_incomplete');
  end if;

  if nullif(v_branch.state,'') is null
     or nullif(v_branch.ibge_city_code,'') is null
     or nullif(v_branch.city,'') is null
     or nullif(v_branch.street,'') is null
     or nullif(v_branch.number,'') is null
     or nullif(v_branch.district,'') is null
     or nullif(v_branch.postal_code,'') is null then
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'product',jsonb_build_object(
        ...
        'fiscal_profile',coalesce(p.fiscal_profile,'{}'::jsonb)
      )
    )
  ...
  if jsonb_array_length(v_items)=0 then
    return jsonb_build_object('ok',false,'error','sale_without_items');
  end if;

  select coalesce(jsonb_agg(
    jsonb_build_object(
      'method',p.method,'status',p.status,'amount',p.amount,
      'tendered_amount',p.tendered_amount,'change_amount',p.change_amount,
      'provider',p.provider,'external_id',p.external_id,'txid',p.txid,
      'metadata',coalesce(p.metadata,'{}'::jsonb)
    )
    order by p.created_at
  ) filter (where p.status in ('approved','paid','completed') or p.status is null),'[]'::jsonb)
  into v_payments
  from public.payments p
  where p.sale_id=v_sale.id;

  select to_jsonb(cu) into v_customer
  from public.customers cu
  where cu.id=v_sale.customer_id;

  select secret into v_key from private.fiscal_crypto_keys where id=1;
  if v_key is null then
    return jsonb_build_object('ok',false,'error','fiscal_crypto_key_missing');
  end if;

  update public.fiscal_documents
  set status='processing',provider='svrs_direct',updated_at=now(),
  ...
  return jsonb_build_object(
    ...
    'certificate',jsonb_build_object(
      ...
      'filename',v_cert.filename,
      'subject_cn',v_cert.subject_cn,
      'subject_cnpj',v_cert.subject_cnpj,
      'serial_number',v_cert.serial_number,
...
create or replace function public.pdv_pull_v4(
  p_device_token text,
...
declare
  ...
  v_cert_json jsonb;
begin
  v_data := public.pdv_pull(p_device_token,p_since);
  if not coalesce((v_data->>'ok')::boolean,false) then return v_data; end if;

  select coalesce(jsonb_agg(p.obj || jsonb_build_object('is_weighable',coalesce(pr.is_weighable,false))),'[]'::jsonb)
    into v_products
  from jsonb_array_elements(coalesce(v_data->'products','[]'::jsonb)) p(obj)
  left join public.products pr on pr.id=(p.obj->>'id')::uuid;

  select * into v_dev from private.resolve_pdv_device(p_device_token);
  select * into v_cert
    from private.fiscal_certificates
    where tenant_id=v_dev.tenant_id and company_id=v_dev.company_id;

  v_cert_json := jsonb_build_object(
    'configured',v_cert.company_id is not null,
    'filename',v_cert.filename,
    'subject_cn',v_cert.subject_cn,
    'subject_cnpj',v_cert.subject_cnpj,
    'serial_number',v_cert.serial_number,
    ...
    'valid_to',v_cert.valid_to,
    'fingerprint_sha256',v_cert.fingerprint_sha256,
    'expired',case when v_cert.company_id is null then false when v_cert.valid_to is null then false else v_cert.valid_to<=now() end,
    'can_emit',v_cert.company_id is not null and (v_cert.valid_to is null or v_cert.valid_to>now())
  );
  ...
  v_data := jsonb_set(
    v_data,
    '{context}',
    coalesce(v_data->'context','{}'::jsonb)
      || jsonb_build_object('fiscal_certificate',v_cert_json,'fiscal_engine','thorfiscal_svrs_v1'),
    true
  );
