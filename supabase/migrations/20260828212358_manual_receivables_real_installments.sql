...
begin
  ...
  if v_account is null then
    select default_chart_account_id into v_account from public.financial_categories where id=v_category;
  end if;
  ...
  if jsonb_typeof(v_schedule)<>'array' then
    return jsonb_build_object('ok',false,'error','invalid_payment_installments');
  end if;
  ...
  if v_inst_count=0 then
    ...
    v_schedule:=jsonb_build_array(jsonb_build_object('due_date',v_due,'amount',v_total_amount));
    v_inst_count:=1;
  ...
  for v_inst in select * from jsonb_array_elements(v_schedule) loop
    ...
    insert into public.financial_entries(
      ...
      case when v_inst_count>1 then v_desc||' - Parcela '||v_inst_no||'/'||v_inst_count else v_desc end,
      v_inst_amount,0,v_due,v_customer.id,null,
      jsonb_build_object(
        'origin','manual_receivable',
        'term_method',v_doc,
        'receivable_group_id',v_group_id,
        'installment',v_inst_no,
        'installments',v_inst_count,
        'group_total_amount',v_total_amount,
  ...
  return jsonb_build_object(
    ...
    'installments',v_inst_count,
    'financial_category_id',v_category,
    'chart_account_id',v_account,
    'cost_center_id',v_cc,
