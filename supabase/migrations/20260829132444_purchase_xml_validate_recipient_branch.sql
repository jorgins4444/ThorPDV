...
declare v record;v_sup jsonb;v_prod jsonb;v_links jsonb;v_units jsonb;v_branch_doc text;v_branch_name text;v_company_doc text;v_company_name text;
begin
 ...
 select regexp_replace(coalesce(b.cnpj,''),'\D','','g'),b.name,regexp_replace(coalesce(c.cnpj,''),'\D','','g'),coalesce(c.trade_name,c.legal_name)
 into v_branch_doc,v_branch_name,v_company_doc,v_company_name
 from branches b join companies c on c.id=b.company_id where b.id=v.branch_id and b.tenant_id=v.tenant_id;
 ...
 select coalesce(jsonb_agg(to_jsonb(p) order by p.name,p.variant_label),'[]'::jsonb) into v_prod from (
  ...
  from products p where p.tenant_id=v.tenant_id and p.active=true and (p.parent_product_id is not null or coalesce(p.product_structure,'simple')<>'grade') limit 1500) p;
 ...
 return jsonb_build_object('ok',true,'suppliers',v_sup,'products',v_prod,'links',v_links,'units',v_units,'branch_id',v.branch_id,'branch_document',v_branch_doc,'branch_name',v_branch_name,'company_document',v_company_doc,'company_name',v_company_name);
end $function$;

create or replace function public.erp_purchase_xml_import(p_token text,p_payload jsonb)
returns jsonb language plpgsql security definer set search_path='public','private' as $function$
declare
 v record;v_supplier uuid;v_doc text;v_dest_doc text;v_expected_doc text;v_s jsonb;v_item jsonb;v_product uuid;v_result jsonb;v_purchase uuid;v_purchase_payload jsonb;v_items jsonb:='[]'::jsonb;v_new jsonb;v_barcode text;v_unit text;v_sale numeric;v_factor numeric;v_source_code text;v_key text;v_failed jsonb:=null;v_updated int;
begin
 select * into v from private.resolve_temp_context(p_token);if v.user_id is null then return jsonb_build_object('ok',false,'error','invalid_session');end if;
 select regexp_replace(coalesce(b.cnpj,''),'\D','','g') into v_expected_doc from branches b where b.id=v.branch_id and b.tenant_id=v.tenant_id;
 if coalesce(v_expected_doc,'')='' then return jsonb_build_object('ok',false,'error','branch_cnpj_not_configured');end if;
 v_dest_doc:=regexp_replace(coalesce(p_payload->>'destination_document',''),'\D','','g');
 if length(v_dest_doc)<>14 then return jsonb_build_object('ok',false,'error','invalid_destination_cnpj');end if;
 if v_dest_doc<>v_expected_doc then return jsonb_build_object('ok',false,'error','nfe_destination_mismatch','expected',v_expected_doc,'received',v_dest_doc);end if;
 ...
 begin
  ...
  if v_supplier is not null and not exists(select 1 from suppliers where id=v_supplier and tenant_id=v.tenant_id and active=true) then v_failed:=jsonb_build_object('ok',false,'error','supplier_not_found');raise exception 'xml_import_abort';end if;
  ...
  for v_item in select * from jsonb_array_elements(p_payload->'items') loop
   ...
   if coalesce(nullif(v_item->>'quantity','')::numeric,0)<=0 or coalesce(nullif(v_item->>'unit_cost','')::numeric,-1)<0 or v_sale<=0 then v_failed:=jsonb_build_object('ok',false,'error','invalid_xml_item_values','source_code',v_source_code);raise exception 'xml_import_abort';end if;
   if v_product is null then
    ...
    if not coalesce((v_new->>'ok')::boolean,false) then v_failed:=jsonb_build_object('ok',false,'error','xml_product_create_failed','source_code',v_source_code,'detail',v_new);raise exception 'xml_import_abort';end if;v_product:=(v_new->>'id')::uuid;
   elsif not exists(select 1 from products where id=v_product and tenant_id=v.tenant_id and active=true) then v_failed:=jsonb_build_object('ok',false,'error','product_not_found','source_code',v_source_code);raise exception 'xml_import_abort';end if;
   ...
   if nullif(v_item->>'source_unit','') is not null and upper(v_item->>'source_unit')<>v_unit then
    update product_purchase_units set conversion_factor=v_factor,barcode=coalesce(v_barcode,barcode),is_default=true,updated_at=now() where tenant_id=v.tenant_id and product_id=v_product and upper(unit)=upper(v_item->>'source_unit');get diagnostics v_updated=row_count;
    if v_updated=0 then insert into product_purchase_units(tenant_id,product_id,unit,conversion_factor,barcode,is_default) values(v.tenant_id,v_product,upper(v_item->>'source_unit'),v_factor,v_barcode,true);end if;
   end if;
  end loop;
  ...
  v_result:=public.erp_purchase_create(p_token,v_purchase_payload);if not coalesce((v_result->>'ok')::boolean,false) then v_failed:=v_result;raise exception 'xml_import_abort';end if;v_purchase:=(v_result->>'purchase_id')::uuid;
  return v_result||jsonb_build_object('supplier_id',v_supplier,'source','nfe_xml','nfe_access_key',v_key,'destination_document',v_dest_doc);
 exception when unique_violation then return jsonb_build_object('ok',false,'error','nfe_already_imported');when others then if v_failed is not null then return v_failed;end if;return jsonb_build_object('ok',false,'error','xml_import_failed','detail',sqlerrm);end;
end $function$;
